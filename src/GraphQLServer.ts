...
const schemaSource = `
type Query {
  textMarkdownSHA256(sha256: String): String
}
`;
...
type FieldResolverFunc = (root: Root, args: Record<string, any>, context: Context) => any | Promise<any>;

const resolversMap = {
  Query: {
    async textMarkdownSHA256(root: Root, { sha256 }: Record<string, any>, context: Context): Promise<string | null> {
...
function resolver(source: any, args: Record<string, any>, context: Context, info: GraphQLResolveInfo) {
  const fieldName = info.fieldName;
  const parentName = info.parentType.name;
  console.log('parentName', parentName)
  return resolversMap[parentName][fieldName](info.rootValue, args, context)
}
...
async function getQuery(request: Request): Promise<string> {
  ...
  if (url.searchParams.has("query")) {
    ...
    } else if (contentType === 'application/graphql' || /^text\/plain/.test(contentType)) {
      return request.text();
...
export async function handleRequest(request: Request): Promise<Response> {
  ...
  const result: ExecutionResult<ExecutionResultDataDefault> = await execute(schema, documentAST, {}, {}, {}, null, resolver);
...
function jsonResponse(json: JSONResponseInput, status: 200 | 400 | 500 = 200): Response {
...
function errorsResponse(status: 400, errors: ReadonlyArray<GraphQLError>): Response {
