...
export function makeColorPalette(): Array<ColorPalette> {
  const out: Array<ColorPalette> = [];

  (Object.keys(defaultTheme.colors) as Array<
    keyof typeof defaultTheme.colors
  >).forEach(baseName => {
    const value = defaultTheme.colors[baseName];
